> Tracked copy of `tests/roms/AccuracyCoin/outcome.md`. That directory is an untracked
> nested git clone of upstream AccuracyCoin (it has its own `.git`, so the outer repo
> silently refuses to track files inside it). If you update one copy, update the other.
