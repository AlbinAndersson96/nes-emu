...
(This file lives in `docs/` — the ROM itself is the `tests/roms/AccuracyCoin`
...
own walkthrough, implemented in two steps:

- **Clear timing (code 1):** a $2002 read latches the VBL flag at the START of the read
  cycle (M2 rise) but samples the sprite-0/overflow bits ~1.9 dots later at its END —
  a read straddling the pre-render-dot-1 clear sees VBL still set with the sprite bits
  already clear. The Bus's $2002 path now re-samples bits 6-5 one dot after the
  (race-calibrated, untouchable) VBL sample point (`Ppu::resample_sprite_flags`, called
  after the existing trailing pre-advance dot — zero change to the 9-dot/3-cycle
  budget, so no alignment risk).
- **Set timing (code 2):** with the reads now sampling sprite bits later, both flags
  became visible one consecutive-dot read too early (found by temporarily dumping the
  test's ZP $51-$57 result bytes from the harness: `00 00 00 60 …` where the key
  demands `00 00 [00|40] 60`). The SET path needs one more dot of latency than the
  clear: `SPRITE0_HIT_SET_LATCH_DOTS` = 2 (clear stays at
  `SPRITE0_HIT_LATCH_DOTS` = 1 — a shared latch of 2 regressed the clear sub-test),
  and the overflow flag gains the same set-side visibility latch
  (`sprite_overflow_set_pending` = 2; its pre-render clear stays immediate). The
  ROM's key encodes the order: a straddling read may see the hit without overflow,
  never the reverse — matching hit-then-overflow visibility. Still inside blargg
  sprite_hit_tests' 0-2 calibration window; the full suite stayed green at every step.

`INC $4014` returned to passing in the same run (the usual phase bounce, in our favor).
Net +2 = 119 → 121.
...
## Remaining failures (16 failing + 4 hung = 20 non-passing), by cluster

Codes are the ROM's on-screen error codes; meanings from `README.md`. Table regenerated
2026-07-20 at the post-Session-11 state.
...
`2002 Flag Clear Timing` fixed in Session 11.)
