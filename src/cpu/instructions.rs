...
pub fn execute(cpu: &mut Cpu, bus: &mut dyn Bus, opcode: u8) -> u8 {
    match opcode {
        // --- ADC ---
        0x69 => { let v = cpu.fetch(bus);          adc(cpu, v);        2 }
        0x65 => { let (a,_) = cpu.addr_zero_page(bus);   adc_m(cpu,bus,a);  3 }
        0x75 => { let (a,_) = cpu.addr_zero_page_x(bus); adc_m(cpu,bus,a);  4 }
        0x6D => { let (a,_) = cpu.addr_absolute(bus);    adc_m(cpu,bus,a);  4 }
        0x7D => { let (a,p) = cpu.addr_absolute_x(bus);  adc_m(cpu,bus,a);  4 + p as u8 }
        0x79 => { let (a,p) = cpu.addr_absolute_y(bus);  adc_m(cpu,bus,a);  4 + p as u8 }
        0x61 => { let (a,_) = cpu.addr_indirect_x(bus);  adc_m(cpu,bus,a);  6 }
        0x71 => { let (a,p) = cpu.addr_indirect_y(bus);  adc_m(cpu,bus,a);  5 + p as u8 }

        // --- AND ---
        0x29 => { let v = cpu.fetch(bus);          and(cpu, v);        2 }
        0x25 => { let (a,_) = cpu.addr_zero_page(bus);   and_m(cpu,bus,a);  3 }
        0x35 => { let (a,_) = cpu.addr_zero_page_x(bus); and_m(cpu,bus,a);  4 }
        0x2D => { let (a,_) = cpu.addr_absolute(bus);    and_m(cpu,bus,a);  4 }
        0x3D => { let (a,p) = cpu.addr_absolute_x(bus);  and_m(cpu,bus,a);  4 + p as u8 }
        0x39 => { let (a,p) = cpu.addr_absolute_y(bus);  and_m(cpu,bus,a);  4 + p as u8 }
        0x21 => { let (a,_) = cpu.addr_indirect_x(bus);  and_m(cpu,bus,a);  6 }
        0x31 => { let (a,p) = cpu.addr_indirect_y(bus);  and_m(cpu,bus,a);  5 + p as u8 }

        // --- ASL ---
        0x0A => {                                  cpu.a = asl(cpu, cpu.a); 2 }
        ...
        // --- BIT ---
        0x24 => { let (a,_) = cpu.addr_zero_page(bus); bit(cpu,bus,a); 3 }
        ...
        0xC5 => { let (a,_) = cpu.addr_zero_page(bus);   cmp_m(cpu,bus,cpu.a,a); 3 }
        0xD5 => { let (a,_) = cpu.addr_zero_page_x(bus); cmp_m(cpu,bus,cpu.a,a); 4 }
        0xCD => { let (a,_) = cpu.addr_absolute(bus);    cmp_m(cpu,bus,cpu.a,a); 4 }
        0xDD => { let (a,p) = cpu.addr_absolute_x(bus);  cmp_m(cpu,bus,cpu.a,a); 4 + p as u8 }
        0xD9 => { let (a,p) = cpu.addr_absolute_y(bus);  cmp_m(cpu,bus,cpu.a,a); 4 + p as u8 }
        0xC1 => { let (a,_) = cpu.addr_indirect_x(bus);  cmp_m(cpu,bus,cpu.a,a); 6 }
        0xD1 => { let (a,p) = cpu.addr_indirect_y(bus);  cmp_m(cpu,bus,cpu.a,a); 5 + p as u8 }

        // --- CPX ---
        0xE0 => { let v = cpu.fetch(bus);         cmp(cpu, cpu.x, v);     2 }
        0xE4 => { let (a,_) = cpu.addr_zero_page(bus);  cmp_m(cpu,bus,cpu.x,a); 3 }
        0xEC => { let (a,_) = cpu.addr_absolute(bus);   cmp_m(cpu,bus,cpu.x,a); 4 }

        // --- CPY ---
        0xC0 => { let v = cpu.fetch(bus);         cmp(cpu, cpu.y, v);     2 }
        0xC4 => { let (a,_) = cpu.addr_zero_page(bus);  cmp_m(cpu,bus,cpu.y,a); 3 }
        0xCC => { let (a,_) = cpu.addr_absolute(bus);   cmp_m(cpu,bus,cpu.y,a); 4 }
        ...
        0x88 => { cpu.y = cpu.y.wrapping_sub(1); cpu.set_nz(cpu.y); 2 }

        // --- EOR ---
        0x49 => { let v = cpu.fetch(bus);          eor(cpu, v);        2 }
        0x45 => { let (a,_) = cpu.addr_zero_page(bus);   eor_m(cpu,bus,a);  3 }
        0x55 => { let (a,_) = cpu.addr_zero_page_x(bus); eor_m(cpu,bus,a);  4 }
        0x4D => { let (a,_) = cpu.addr_absolute(bus);    eor_m(cpu,bus,a);  4 }
        0x5D => { let (a,p) = cpu.addr_absolute_x(bus);  eor_m(cpu,bus,a);  4 + p as u8 }
        0x59 => { let (a,p) = cpu.addr_absolute_y(bus);  eor_m(cpu,bus,a);  4 + p as u8 }
        0x41 => { let (a,_) = cpu.addr_indirect_x(bus);  eor_m(cpu,bus,a);  6 }
        0x51 => { let (a,p) = cpu.addr_indirect_y(bus);  eor_m(cpu,bus,a);  5 + p as u8 }
        ...
        0xC8 => { cpu.y = cpu.y.wrapping_add(1); cpu.set_nz(cpu.y); 2 }

        // --- JMP ---
        0x4C => { cpu.pc = cpu.fetch_u16(bus); 3 }
        0x6C => { let ptr = cpu.fetch_u16(bus); cpu.pc = cpu.read_u16_bugged(bus, ptr); 5 }

        // --- JSR ---
        0x20 => { jsr(cpu, bus); 6 }

        // --- LDA ---
        0xA9 => { let v = cpu.fetch(bus);          lda(cpu, v);       2 }
        0xA5 => { let (a,_) = cpu.addr_zero_page(bus);   lda_m(cpu,bus,a); 3 }
        0xB5 => { let (a,_) = cpu.addr_zero_page_x(bus); lda_m(cpu,bus,a); 4 }
        0xAD => { let (a,_) = cpu.addr_absolute(bus);    lda_m(cpu,bus,a); 4 }
        0xBD => { let (a,p) = cpu.addr_absolute_x(bus);  lda_m(cpu,bus,a); 4 + p as u8 }
        0xB9 => { let (a,p) = cpu.addr_absolute_y(bus);  lda_m(cpu,bus,a); 4 + p as u8 }
        0xA1 => { let (a,_) = cpu.addr_indirect_x(bus);  lda_m(cpu,bus,a); 6 }
        0xB1 => { let (a,p) = cpu.addr_indirect_y(bus);  lda_m(cpu,bus,a); 5 + p as u8 }

        // --- LDX ---
        0xA2 => { let v = cpu.fetch(bus);          ldx(cpu, v);       2 }
        0xA6 => { let (a,_) = cpu.addr_zero_page(bus);   ldx_m(cpu,bus,a); 3 }
        0xB6 => { let (a,_) = cpu.addr_zero_page_y(bus); ldx_m(cpu,bus,a); 4 }
        0xAE => { let (a,_) = cpu.addr_absolute(bus);    ldx_m(cpu,bus,a); 4 }
        0xBE => { let (a,p) = cpu.addr_absolute_y(bus);  ldx_m(cpu,bus,a); 4 + p as u8 }

        // --- LDY ---
        0xA0 => { let v = cpu.fetch(bus);          ldy(cpu, v);       2 }
        0xA4 => { let (a,_) = cpu.addr_zero_page(bus);   ldy_m(cpu,bus,a); 3 }
        0xB4 => { let (a,_) = cpu.addr_zero_page_x(bus); ldy_m(cpu,bus,a); 4 }
        0xAC => { let (a,_) = cpu.addr_absolute(bus);    ldy_m(cpu,bus,a); 4 }
        0xBC => { let (a,p) = cpu.addr_absolute_x(bus);  ldy_m(cpu,bus,a); 4 + p as u8 }

        // --- LSR ---
        0x4A => { cpu.a = lsr(cpu, cpu.a); 2 }
        ...
        // --- ORA ---
        0x09 => { let v = cpu.fetch(bus);          ora(cpu, v);        2 }
        0x05 => { let (a,_) = cpu.addr_zero_page(bus);   ora_m(cpu,bus,a);  3 }
        0x15 => { let (a,_) = cpu.addr_zero_page_x(bus); ora_m(cpu,bus,a);  4 }
        0x0D => { let (a,_) = cpu.addr_absolute(bus);    ora_m(cpu,bus,a);  4 }
        0x1D => { let (a,p) = cpu.addr_absolute_x(bus);  ora_m(cpu,bus,a);  4 + p as u8 }
        0x19 => { let (a,p) = cpu.addr_absolute_y(bus);  ora_m(cpu,bus,a);  4 + p as u8 }
        0x01 => { let (a,_) = cpu.addr_indirect_x(bus);  ora_m(cpu,bus,a);  6 }
        0x11 => { let (a,p) = cpu.addr_indirect_y(bus);  ora_m(cpu,bus,a);  5 + p as u8 }
        ...
        // --- SBC ---
        0xE9 => { let v = cpu.fetch(bus);          sbc(cpu, v);        2 }
        0xE5 => { let (a,_) = cpu.addr_zero_page(bus);   sbc_m(cpu,bus,a);  3 }
        0xF5 => { let (a,_) = cpu.addr_zero_page_x(bus); sbc_m(cpu,bus,a);  4 }
        0xED => { let (a,_) = cpu.addr_absolute(bus);    sbc_m(cpu,bus,a);  4 }
        0xFD => { let (a,p) = cpu.addr_absolute_x(bus);  sbc_m(cpu,bus,a);  4 + p as u8 }
        0xF9 => { let (a,p) = cpu.addr_absolute_y(bus);  sbc_m(cpu,bus,a);  4 + p as u8 }
        0xE1 => { let (a,_) = cpu.addr_indirect_x(bus);  sbc_m(cpu,bus,a);  6 }
        0xF1 => { let (a,p) = cpu.addr_indirect_y(bus);  sbc_m(cpu,bus,a);  5 + p as u8 }
        ...
        0x96 => { let (a,_) = cpu.addr_zero_page_y(bus); let v=cpu.x; cpu.write(bus,a,v); 4 }
        0x8E => { let (a,_) = cpu.addr_absolute(bus);    let v=cpu.x; cpu.write(bus,a,v); 4 }

        // --- STY ---
        0x84 => { let (a,_) = cpu.addr_zero_page(bus);   let v=cpu.y; cpu.write(bus,a,v); 3 }
        0x94 => { let (a,_) = cpu.addr_zero_page_x(bus); let v=cpu.y; cpu.write(bus,a,v); 4 }
        0x8C => { let (a,_) = cpu.addr_absolute(bus);    let v=cpu.y; cpu.write(bus,a,v); 4 }

        // --- Transfers ---
        0xAA => { cpu.x = cpu.a; cpu.set_nz(cpu.x); 2 } // TAX
        0xA8 => { cpu.y = cpu.a; cpu.set_nz(cpu.y); 2 } // TAY
        0xBA => { cpu.x = cpu.sp; cpu.set_nz(cpu.x); 2 } // TSX
        0x8A => { cpu.a = cpu.x; cpu.set_nz(cpu.a); 2 } // TXA
        0x9A => { cpu.sp = cpu.x; 2 }                    // TXS — no flags
        0x98 => { cpu.a = cpu.y; cpu.set_nz(cpu.a); 2 } // TYA
        ...
        0x80 | 0x82 | 0x89 | 0xC2 | 0xE2 => { cpu.fetch(bus); 2 }

        // DOP — 2-byte NOPs (zero page)
        0x04 | 0x44 | 0x64 => { cpu.addr_zero_page(bus); 3 }

        // DOP — 2-byte NOPs (zero page,X)
        0x14 | 0x34 | 0x54 | 0x74 | 0xD4 | 0xF4 => { cpu.addr_zero_page_x(bus); 4 }

        // TOP — 3-byte NOP (absolute)
        0x0C => { cpu.addr_absolute(bus); 4 }
        ...
        0xEB => { let v = cpu.fetch(bus); sbc(cpu, v); 2 }
        ...
        // --- SAX (Store A & X) ---
        0x87 => { let (a,_) = cpu.addr_zero_page(bus);   sax(cpu,bus,a); 3 }
        0x97 => { let (a,_) = cpu.addr_zero_page_y(bus); sax(cpu,bus,a); 4 }
        0x8F => { let (a,_) = cpu.addr_absolute(bus);    sax(cpu,bus,a); 4 }
        0x83 => { let (a,_) = cpu.addr_indirect_x(bus);  sax(cpu,bus,a); 6 }

        // --- LAX (Load A and X from memory) ---
        0xA7 => { let (a,_) = cpu.addr_zero_page(bus);   lax_m(cpu,bus,a); 3 }
        0xB7 => { let (a,_) = cpu.addr_zero_page_y(bus); lax_m(cpu,bus,a); 4 }
        0xAF => { let (a,_) = cpu.addr_absolute(bus);    lax_m(cpu,bus,a); 4 }
        0xBF => { let (a,p) = cpu.addr_absolute_y(bus);  lax_m(cpu,bus,a); 4 + p as u8 }
        0xA3 => { let (a,_) = cpu.addr_indirect_x(bus);  lax_m(cpu,bus,a); 6 }
        0xB3 => { let (a,p) = cpu.addr_indirect_y(bus);  lax_m(cpu,bus,a); 5 + p as u8 }

        // LAX #imm (ATX) — load both A and X with the immediate value
        0xAB => { let v = cpu.fetch(bus); lax(cpu, v); 2 }

        // --- AXS / SBX (A & X minus immediate, result into X) ---
        0xCB => { let v = cpu.fetch(bus); axs(cpu, v); 2 }
        ...
        0x0B | 0x2B => { let v = cpu.fetch(bus); anc(cpu, v); 2 }

        // --- ALR (AND #n then LSR accumulator) ---
        0x4B => { let v = cpu.fetch(bus); alr(cpu, v); 2 }

        // --- ARR (AND #n then ROR accumulator, special V/C) ---
        0x6B => { let v = cpu.fetch(bus); arr(cpu, v); 2 }

        // --- ANE / XAA (unstable; A = X & imm) ---
        0x8B => { let v = cpu.fetch(bus); cpu.a = cpu.x & v; cpu.set_nz(cpu.a); 2 }

        // --- LAS (AND memory with SP, load all three) ---
        0xBB => { let (a,p) = cpu.addr_absolute_y(bus); las(cpu,bus,a); 4 + p as u8 }
        ...
        0x93 => {
            ...
            let addr_hi = if p { (a >> 8) as u8 - 1 } else { (a >> 8) as u8 };
        ...
        0x9C => {
            let (a, p) = cpu.addr_absolute_x(bus);
            let operand_hi = if p { (a >> 8) as u8 - 1 } else { (a >> 8) as u8 };
            let v = cpu.y & operand_hi.wrapping_add(1);
            let write_addr = if p { ((operand_hi as u16) << 8) | (a & 0x00FF) } else { a };
            cpu.write(bus, write_addr, v); 5
        }
...
fn lda(cpu: &mut Cpu, v: u8) { cpu.a = v; cpu.set_nz(v); }
