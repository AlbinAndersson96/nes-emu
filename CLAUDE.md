...
- **`src/tests/cartridge.rs`** — mapper unit tests against synthetic in-memory iNES images (UxROM/AxROM banking and mirroring — the discrete-logic mappers with no blargg ROM suite in tests/roms/).
...
- **`src/tests/accuracycoin.rs`** — automated headless runner for the AccuracyCoin all-in-one accuracy ROM (`tests/roms/AccuracyCoin/`, tracked directly in this repo): boots the ROM, injects a Start press (runs all 141 tests), and dumps every verdict from its fixed RAM address ($0400–$0492, plus the DMA-sync pre-test byte at ZP $12). `#[ignore]`d — run with `cargo test accuracycoin --release -- --ignored --nocapture` (currently takes the full ~70s `MAX_CYCLES` timeout to complete, due to the `Implied Dummy Reads` hang noted below). Results, fixes, and remaining-failure analysis live in `tests/roms/AccuracyCoin/outcome.md`. `TRACE_DMC_DMA=1` additionally logs every DMC-DMA halt (in-flight address, stall length, exact tick stamp).
...
**DMC DMA cycle accuracy**: DMC DMA is serviced synchronously, mid-instruction, inside `CpuBus::read` (`Bus::maybe_dmc_dma`): a request that asserted on an EARLIER cycle halts the current read (hardware RDY sampling — never the assert cycle itself, never a write cycle), performs 2–3 side-effecting dummy re-reads of the in-flight address plus the sample fetch, and reports the stolen cycles to the CPU via `CpuBus::take_dma_stall_cycles` (drained into `Cpu::cycles` at every `Cpu::tick` exit point, so `SystemClock` ticks the PPU/APU for them automatically). Detection uses a DMC-only real-time clock (`Apu::dmc_tick_realtime`, one tick per bus access) with a `dmc_debt` counter so the post-hoc `Apu::tick_one` never double-clocks the channel. Calibrated constants (all swept against AccuracyCoin page 13; see `tests/roms/AccuracyCoin/outcome.md`): `DMC_DMA_ALIGNED_PARITY = true` (3- vs 4-cycle stall from `Apu::dmc_realtime_current_parity()` — the live phase, NOT the instruction-start-stale `cycle_parity()`), `DMC_LOAD_DMA_DELAY = 3` (enable-started load DMAs halt no earlier than the 4th cycle after the $4015 write; reload DMAs are not delayed). Channel-timer clocking is anchored to the free-running `cycle_count` — never `frame_cycles`, which resets on $4017 writes and would flip the get/put phase. The real-time clock assumes one bus access per CPU cycle; JSR, RTS, RTI, PLA, and PLP now each perform exactly the real hardware-documented bus-access sequence for every cycle (fixed via `src/tests/cpu.rs`'s `*_bus_access_sequence` tests — see "Hardware cycle-by-cycle accuracy" below), closing the JSR/RTS-heavy-code drift that used to affect most of the AccuracyCoin DMA cluster. KNOWN REMAINING LIMIT: one AccuracyCoin test (`Implied Dummy Reads`) still hangs — its "Test 5" executes code fetched from open bus, precisely timed so a DMC DMA's fetched byte becomes the "opcode" at an APU register address; our DMA-during-JSR interaction is accurate enough for every other DMA test but not quite exact enough for this one sub-cycle-exact trick, causing it to land on unexpected garbage and recurse instead of reaching its designed BRK/RTI pass/fail path. Not root-caused; see `tests/roms/AccuracyCoin/outcome.md`'s "Session 2" section for the full investigation (DMC-channel/SP/PC evidence) and fix direction.

**Hardware cycle-by-cycle accuracy (JSR/RTS/RTI/PLA/PLP)**: real 6502 hardware drives the address bus on every cycle of every instruction, including cycles whose result is discarded ("internal operations") — there is no such thing as a cycle with no bus access. `src/cpu/instructions.rs`'s `jsr`/`rts`/`rti` functions and the `0x68`(PLA)/`0x28`(PLP) match arms perform an explicit dummy `bus.read` for each such cycle (a `$0100|SP` stack peek for RTS/RTI/PLA/PLP's "increment/decrement S" cycle, plus RTS's cycle-6 read of the popped target address before the final `+1`; JSR was also reordered to fetch its high operand byte *last*, matching hardware — previously both operand bytes were fetched up front, before the two pushes). This matters beyond DMC DMA: it's also what makes the CPU open-bus latch (`Bus::cpu_open_bus`) hold the right value after these instructions. `src/tests/cpu.rs`'s `jsr_bus_access_sequence`, `rts_bus_access_sequence`, `rti_bus_access_sequence`, `pla_bus_access_sequence`, `plp_bus_access_sequence`, and `pha_bus_access_sequence` (a regression check — PHA/PHP needed no fix) assert the exact `(address, is_write)` sequence via `TestBus.trace`. BRK needed no fix either — its interrupt-sequence micro-ops (`PushPcHi`/`PushPcLo`/`PushP`/`VectorFetch`/`VectorFetchHi`, `src/cpu/mod.rs`) already perform one real bus access per cycle.
...
**`cargo test` is fully green**: every wired-up blargg suite passes and asserts its ROM's own verdict (308 tests; the single `#[ignore]`d one is the JAM-opcode ROM that can never finish). This section records what was fixed and where the remaining unwired/unsupported territory is (the APU suites, mappers beyond 0/1/2/3/4/7).
...
`tests/roms/AccuracyCoin/` (tracked directly in this repo) holds the 141-test all-in-one accuracy ROM. Current standing: **100/141 pass** on this branch (was 91 on develop) — same raw count across both the DMC-DMA fix session and the later JSR/RTS silent-cycle-fix session, but the composition shifted (one test fixed, one newly broken, four downgraded from a clean fail to a hang — see `tests/roms/AccuracyCoin/outcome.md`'s "Session 2" section before assuming "100" means "nothing changed"). The DMC-DMA cluster was largely fixed first (timer period off-by-one, halt-after-assert, cycle_count parity anchor, live stall parity, 3-cycle load-DMA delay), then JSR/RTS/RTI/PLA/PLP were made fully cycle-accurate (see "Hardware cycle-by-cycle accuracy" above) — which fixed DMA + $4015 Read but also exposed a deeper, unresolved DMA-during-JSR precision issue in `Implied Dummy Reads`' open-bus-execution trick (now hangs instead of failing cleanly, taking 3 other tests down as collateral since they run after it). `tests/roms/AccuracyCoin/outcome.md` documents every remaining failure with its error code and the three items deliberately out of scope (DMC bus conflicts, DMC+OAM DMA overlap, explicit/implicit DMA abort). Read outcome.md before resuming this work.
