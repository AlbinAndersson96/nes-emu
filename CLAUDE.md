...
- **`src/tests/accuracycoin.rs`** — automated headless runner for the AccuracyCoin all-in-one accuracy ROM (`tests/roms/AccuracyCoin/`): boots the ROM, injects a Start press (runs all 141 tests), and dumps every verdict from its fixed RAM address ($0400–$0492, plus the DMA-sync pre-test byte at ZP $12). `#[ignore]`d — run with `cargo test accuracycoin --release -- --ignored --nocapture`. Results, fixes, and remaining-failure analysis live in `tests/roms/AccuracyCoin/outcome.md` (NOTE: that directory is a nested git clone of upstream AccuracyCoin, so the outer repo cannot track files inside it — `git add` skips them silently; the committed copy is `docs/accuracycoin_outcome.md`, keep the two in sync). `TRACE_DMC_DMA=1` additionally logs every DMC-DMA halt (in-flight address, stall length, exact tick stamp).
...
`tests/roms/AccuracyCoin/` (an untracked nested git clone of upstream — files inside it cannot be committed to this repo; the tracked copy of the results doc is `docs/accuracycoin_outcome.md`) holds the 141-test all-in-one accuracy ROM. Current standing: **100/141 pass** on this branch (was 91 on develop). The DMC-DMA cluster was largely fixed here (timer period off-by-one, halt-after-assert, cycle_count parity anchor, live stall parity, 3-cycle load-DMA delay — commits `86da3d9..3acd17b`); `tests/roms/AccuracyCoin/outcome.md` documents every remaining failure with its error code, the JSR/RTS silent-cycle root-cause analysis for the rest of the DMA cluster, and the three items deliberately out of scope (DMC bus conflicts, DMC+OAM DMA overlap, explicit/implicit DMA abort). Read outcome.md before resuming this work.
